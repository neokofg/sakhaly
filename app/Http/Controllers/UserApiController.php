    ...
    protected function updateStat($json){
        ...
        foreach($user as $userItem){
            ...
            if($decodedStats == null){
                ...
                return response(json_encode($findUser[0],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),200);
            }else{
                if (isset($decodedStats[$realTime])) {
                    ...
                    User::where('id',$user_id)->update([
                        'stats' => json_encode($decodedStats,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)
                    ]);
                    $findUser = User::where('id',$user_id)->get();
                    return response(json_encode($findUser[0],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),200);
                }else{
                    ...
                    User::where('id',$user_id)->update([
                        'stats' => json_encode($result,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)
                    ]);
                    $findUser = User::where('id',$user_id)->get();
                    return response(json_encode($findUser[0],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),200);
                }
